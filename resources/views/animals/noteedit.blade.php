    ...
    <div class="container mx-auto p-6">
        <div class="bg-white font-serif dark:bg-gray-800 rounded-lg shadow-lg">
            <div class="p-6 border-b border-gray-200">
                ...
                <form action="{{ route('note.update', ['animal_id' => $animal->id, 'note_id' => $note->id]) }}" method="POST" >
                    @csrf <!-- Add CSRF token -->
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class=" dark:text-gray-200  mb-4 text-gray-600">Content <span class="text-red-500"></span></label>
                            <textarea name="content" class="w-full  h-40 border dark:text-gray-200 dark:bg-gray-700 border-gray-300 rounded-lg p-2" value="{{$note->content}}"  id="content" required></textarea>
                        </div>


                        <div class="col-span-1">
                            <label class="dark:text-gray-200 mb-4  text-gray-600" for="note_date">Date</label>
                            <input class="w-full p-2 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 rounded-lg" value="{{$note->date}}" type="date" name="date" id="note_date">
                        </div>

                        <div class="col-span-1">
                            <label class="dark:text-gray-200  mb-4 text-gray-600" for="note_category">Category</label>
                            <select class="w-full p-2 border dark:bg-gray-700 dark:text-gray-200  border-gray-300 rounded-lg" value="{{$note->category}}" name="category" id="note_category">
                                <option value=""></option>
...
<option value="Other">Other</option></select>
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="input-group">
                                ...
                                <input class="w-full p-2 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 rounded-lg" maxlength="100" size="100" type="text" value="{{$note->keywords}}" name="keywords" id="note_keywords">
                            </div>
                        </div>
                        <div class="col-span-1">
                            <label class="dark:text-gray-200  mb-4 text-gray-600" for="note_time">Time</label>
                            <input class="w-full p-2 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 rounded-lg" value="{{$note->time}}" type="time" name="time" id="note_time">
                        </div>
                        <div class="col-span-1">
                    ...
                    <div class="flex justify-end mt-6">
                        <button type="button" class="px-3 py-2 text-sm mr-4 mb-4 dark:text-gray-100  tracking-wide text-white capitalize transition-colors duration-200 transform bg-red-500 rounded-md dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
                            ...
                            <a href="{{ route('animals.shownote', ['animal_id' => $animal->id]) }}" class="btn btn-gray-500">Cancel</a>
                        </button>
