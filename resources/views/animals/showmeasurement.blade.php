<x-app-layout title="Animal Treatments">
    ...
    <div class=" mt-2 dark:gray-700 ml-2 mr-2 border border-gray-400 text-red-700 px-4 py-3 rounded relative" role="alert">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              ...
              <a href="{{route('AnimalContent.show', $animal->id)}}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{$animal->name}}</a>
            </div>
          </li>
          <li aria-current="page">
            <div class="flex items-center">
              ...
              <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Measurement</span>
            </div>
          </li>
        </ol>
      </nav>
    ...
    <div class="container mx-auto p-6 space-y-6">
        ...
        <div class="mb-4 flex flex-col font-serif sm:flex-row justify-between items-center">
            <div class="mb-2 sm:mb-0">
                <a class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 transition duration-300 ease-in-out" data-remote="true" href="{{ route('animals.measurement', ['animal_id' => $animal->id]) }}">New measurement Record</a>
            </div>
            <div class="sm:ml-2">
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
       ...
       {{--}} <caption class="p-5 text-lg font-semibold text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
        ...
        <thead class="text-xs font-serif text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                    height<span class="text-red-500">(m)</span>
                </th>
                <th scope="col" class="px-6 py-3">
                    weight<span class="text-red-500">(kg)</span>
                </th>
                <th scope="col" class="px-6 py-3">
                    temperature<span class="text-xs text-red-500 align-top">(°C)</span>
                </th>
                <th scope="col" class="px-6 py-3">
                    Action
                </th>
        ...
        <tbody>
            ...
            <tr class="bg-white font-serif  border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                   {{--}} @if ($measurement->temp)
                    {{ $measurement->temp ?: 'N/A'}} <span class="text-xs text-red-500 align-top">°C</span>
    @endif--}}
    {{ $measurement->temp ?: 'N/A'}}
                ...
                <td td class="px-6 py-4">
                    ...
                    <a href="{{ route('measurement.delete', ['animal_id' => $animal->id, 'measurement_id' => $measurement->id]) }}"  class="text-red-600 hover:underline print-hidden">Delete</a>
                </td>
    ...
    <p class="mb-4 text-gray-600 font-serif text-center dark:text-gray-400"> Displaying all <span class="text-red-500">{{ count($measurements) }}</span> records</p>
